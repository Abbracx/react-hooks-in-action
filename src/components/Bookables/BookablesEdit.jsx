...
function useBookable(id) {
  ...
  */
  ...
  return useQuery(
    ...
    {
      // refetchinf causes a probles after deleteing a bookable
  ...
  const mutation = useMutation(
...

    {
      onSuccess: (bookable) => {
        ...
        // do the same for the individuall 'bookable' cache
...
function updateBookablesCache(bookable, queryClient) {
  ...
  if (bookableIndex !== -1) {
    ...
    queryClient.setQueryDate("bookables", bookables);
  }
}
...
function useDeleteBookable() {
  ...
  return {
    deletedBookable: mutation.mutate,
